...
module.exports = function (app) {
    ...
    try {
      ...
      const sanitized = threads.map(t => {
        const replies = (t.replies || [])
          .sort((a, b) => b.created_on - a.created_on) // últimas primero
          .slice(0, 3)
  ...
  app.put('/api/threads/:board', async (req, res) => {
    try {
      ...
      if (!thread_id || !isObjectId(thread_id)) return res.status(400).type('text').send('missing fields');
  ...
  app.get('/api/replies/:board', async (req, res) => {
    try {
      ...
      // Solo campos públicos del hilo y de todas sus replies
      const t = await Thread.findById(thread_id)
        .select('text created_on bumped_on replies._id replies.text replies.created_on')
        .lean();
      ...
      // Devolver TODAS las replies (orden estable opcional: ascendente por fecha)
      const replies = (t.replies || [])
        .map(r => ({ _id: r._id, text: r.text, created_on: r.created_on }))
  ...
  app.put('/api/replies/:board', async (req, res) => {
    try {
      ...
      if (!thread_id || !reply_id || !isObjectId(thread_id) || !isObjectId(reply_id)) {
        return res.status(400).type('text').send('missing fields');
      }
  ...
  app.delete('/api/replies/:board', async (req, res) => {
    try {
      ...
      if (!thread_id || !reply_id || !delete_password || !isObjectId(thread_id) || !isObjectId(reply_id)) {
        return res.status(400).type('text').send('missing fields');
      }
